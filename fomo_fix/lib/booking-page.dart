...
class _BookingPageState extends State<BookingPage> {
  ...
  void _incrementStandard() => setState(() {
    if (_standardTicketCount < _standardTicketsAvailable) _standardTicketCount++;
  });
  void _decrementStandard() => setState(() => _standardTicketCount > 0 ? _standardTicketCount-- : null);
  void _incrementVip() => setState(() {
    if (_vipTicketCount < _vipTicketsAvailable) _vipTicketCount++;
  });
  void _decrementVip() => setState(() => _vipTicketCount > 0 ? _vipTicketCount-- : null);

  @override
  Widget build(BuildContext context) {
    // Calculate total price based on ticket counts
    final double totalPrice = (_standardTicketCount * _standardPrice) + (_vipTicketCount * _vipPrice);
          ...
          Column(
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(color: ticketsLeft > 10 ? Colors.greenAccent : Colors.redAccent, fontSize: 12),
              ),
            ],
          ),
          Container(
  ...
  Widget _buildFloatingBookButton(double totalPrice) {
    return Positioned(
      ...
      right: 0,
      child: Container(
        padding: const EdgeInsets.fromLTRB(16, 16, 16, 32), // More padding for safe area
        decoration: BoxDecoration(
